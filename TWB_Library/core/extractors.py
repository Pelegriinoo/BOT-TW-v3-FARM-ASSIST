...
class Extractor:
    ...
    @staticmethod
    def get_player_name(res):
        """
        Extrai o nome do jogador da página do jogo
        """
        if type(res) != str:
            res = res.text
        
        # Tenta extrair do game data
        game_data = Extractor.game_state(res)
        if game_data and 'player' in game_data and 'name' in game_data['player']:
        ...
        # Procura no header ou menu do jogador
        player_name_elem = soup.select_one('#header_player_name, .header_player_name, [class*="player"], [id*="player"]')
        if player_name_elem:
            return player_name_elem.get_text().strip()
        
        # Procura em links ou elementos com texto do jogador
        for link in soup.find_all('a'):
            href = link.get('href', '')
            if 'screen=info_player' in href and link.get_text().strip():
