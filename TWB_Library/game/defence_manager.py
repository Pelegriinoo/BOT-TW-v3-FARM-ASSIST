...
class DefenceManager:
    ...
    def manage_flags(self):
        ...
        # CORREÇÃO AQUI: Removido a recursão problemática e corrigido a lógica do upgrade
        upgraded = 0
        raw_flags = json.loads(get_flag_data.group(1))
        self.flags = {}
        
        # Primeiro, construir o dicionário de flags disponíveis
        ...
        # Depois, fazer os upgrades necessários
        for flag_type in raw_flags:
            for level in raw_flags[flag_type]:
                for amount in raw_flags[flag_type][level]:
                    # Só faz upgrade se tiver 3+ materiais E se o level for maior que 1
                    # (não pode fazer upgrade do level 0)
                    if int(amount) >= 3 and int(level) > 1:
                        # Corrigido: usar level-1 como from_level para fazer o upgrade correto
                        from_level = int(level) - 1
                        result = self.flag_upgrade(flag=flag_type, level=from_level)
                        
                        # Verificar se o upgrade foi bem sucedido
                        if result and hasattr(result, 'status_code') and result.status_code == 200:
                            self.logger.info("Successfully upgraded flag %s from level %d to %d", 
                                           flag_type, from_level, int(level))
                            upgraded += 1
                        else:
                            self.logger.warning("Failed to upgrade flag %s from level %d", 
                                              flag_type, from_level)
        
        # REMOVIDO: A chamada recursiva problemática
        # if upgraded:
        #     return self.manage_flags()
        
        # Log do resultado final
        if upgraded > 0:
            self.logger.info("Total flags upgraded: %d", upgraded)
        else:
            self.logger.debug("No flags were upgraded this cycle")
